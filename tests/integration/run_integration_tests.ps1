...
foreach ($cfg in $configs) {
	...
	cmake 	-G "$($cfg.Generator)" `
			...
			-Wno-dev `
			"-DUSE_FALLBACK_OPENSSL=$($cfg.UseFallbackOpenSSL)" `
			"-DUSE_FALLBACK_CURL=$($cfg.UseFallbackCurl)" `
			"-DUSE_FALLBACK_ASIO=$($cfg.UseFallbackAsio)" `
			"-DUSE_FALLBACK_SIMPLE_SERVER=$($cfg.UseFallbackSimpleServer)" `
			"-DUSE_FALLBACK_SIMPLE_WS_SERVER=$($cfg.UseFallbackSimpleWsServer)" `
			"-DTEST_PRELOAD_DEPS=$($cfg.TestPreloadDeps)" `
			"-DOPENSSL_SHARED=$($cfg.OpensslShared)" `
			"-DCURL_SHARED=$($cfg.CurlShared)"
