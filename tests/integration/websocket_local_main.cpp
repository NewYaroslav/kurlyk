...
int main() {
    ...
    kurlyk::init(true);
    {
        ...
        client.on_event([&events, &open_status_code, &done, &done_promise, &echoed_message,
                         &accepted_send_callback_called, &rejected_submit_callback_called, &rejected_bool_callback_called]
                        (std::unique_ptr<kurlyk::WebSocketEventData> event) {
